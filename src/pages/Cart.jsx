    ...
    <div className="max-w-4xl mx-auto px-6 pt-10 pb-24">
      ...
      <div className="space-y-4 mb-8">
        {items.map((item) => (
          <div
            key={item.key}
            className="flex items-center gap-4 bg-card border border-line rounded-2xl p-4"
          >
            <div className="w-20 h-20 rounded-xl bg-paper border border-line flex items-center justify-center shrink-0">
            ...
            <p className="font-mono-label w-16 text-right">${item.price * item.qty}</p>
            <button
      ...
      <div className="bg-card border border-line rounded-2xl p-6 flex items-center justify-between">
        <div>
          <p className="text-sm text-muted">Subtotal</p>
          <p className="font-mono-label text-2xl">${subtotal}</p>
        </div>
        <button
          onClick={() => navigate("/checkout")}
          className="flex items-center gap-2 bg-moss text-paper px-6 py-3.5 rounded-full text-sm font-medium hover:bg-moss/90 transition-colors"
        >
